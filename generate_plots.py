    ...
    try:
        ...
        for train_metric, val_metric, title, filename in comparison_pairs:
            ...
            actual_train = (
                ...
                else (train_metric + "(B)" if train_metric + "(B)" in df.columns else None)
            )
            actual_val = (
                val_metric
                if val_metric in df.columns
                else (val_metric + "(B)" if val_metric + "(B)" in df.columns else None)
            )
    ...
    try:
        ...
        for train_col, val_col in column_pairs:
            ...
            actual_train = (
                ...
                else (train_col + "(B)" if train_col + "(B)" in df.columns else None)
            )
            actual_val = (
                val_col if val_col in df.columns else (val_col + "(B)" if val_col + "(B)" in df.columns else None)
            )
        ...
        if mAP50_col:
            ...
            improvement = (
                ((final_mAP50 - df.iloc[0][mAP50_col]) / df.iloc[0][mAP50_col] * 100)
                if df.iloc[0][mAP50_col] > 0
                else 0
            )
