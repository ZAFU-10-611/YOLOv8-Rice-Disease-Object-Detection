...
class YOLODetectGUI:
    ...
    def create_widgets(self):
        ...
        self.conf_slider = ttk.Scale(row4, from_=0.0, to=1.0, orient=tk.HORIZONTAL, variable=self.conf_var, length=200)
        ...
        self.progress_bar = ttk.Progressbar(
            row4, variable=self.progress_var, maximum=100, length=400, mode="determinate"
        )
        ...
        self.result_text = tk.Text(right_frame, height=30, width=40, yscrollcommand=scrollbar.set, wrap=tk.WORD)
        ...
        ttk.Button(nav_frame, text="保存当前结果", command=self.save_current_result, width=15).pack(
            side=tk.LEFT, padx=5
        )
    ...
    def load_model(self):
        """加载YOLO模型."""
        file_path = filedialog.askopenfilename(title="选择模型", filetypes=[("模型文件", "*.pt"), ("所有文件", "*.*")])
    ...
    def select_single_image(self):
        ...
        file_path = filedialog.askopenfilename(
            title="选择图像文件", filetypes=[("图像文件", "*.jpg *.jpeg *.png *.bmp *.gif *.tiff"), ("所有文件", "*.*")]
        )
    ...
    def select_folder(self):
        ...
        if folder:
            ...
            if self.image_files:
                ...
                self.image_source_label.config(text=f"{folder_name} ({len(self.image_files)}张)", foreground="green")
                self.detected_images = []  # 清空之前的检测结果
                self.current_image_idx = 0
                self.current_results = None
                messagebox.showinfo("成功", f"找到 {len(self.image_files)} 张图像")
                self.update_result_text(
                    f"已加载文件夹，共 {len(self.image_files)} 张图像\n\n请点击「批量检测」进行检测"
                )
                self.nav_info_label.config(text=f"共 {len(self.image_files)} 张图像，请开始检测")
            else:
                messagebox.showwarning("提示", "文件夹中未找到图像文件")
    ...
    def select_save_path(self):
        ...
        if folder:
            ...
            messagebox.showinfo("成功", "保存路径已设置")
    ...
    def _detect_all_thread(self):
        ...
        try:
            ...
            for idx, image_path in enumerate(self.image_files):
                ...
                # 保存检测结果图像和结果对象
                self.detected_images.append({"image": result_img, "result": results[0], "path": image_path})
            ...
            if self.detected_images:
                ...
                self.nav_info_label.config(text=f"检测结果: {file_name} (1/{len(self.detected_images)})")
            ...
            self.progress_var.set(0)
            self.progress_label.config(text="0/0")
            messagebox.showerror("错误", f"批量检测失败: {e!s}")
            self.update_result_text(f"✗ 批量检测失败: {e!s}")
    ...
    def prev_image(self):
        ...
        self.nav_info_label.config(text=f"检测结果: {file_name} ({new_idx + 1}/{len(self.detected_images)})")
