...
def parse_opt():
    ...
    parser.add_argument('--plot-style', default='curve', choices=['table', 'curve', 'both'],
    ...
    try:
        ...
        for train_metric, val_metric, title, filename in comparison_pairs:
            if train_metric in df.columns and val_metric in df.columns:
                ...
                # 绘制训练和验证曲线
                plt.plot(epochs, df[train_metric], color='blue', linewidth=2.5,
                         marker='o', markersize=4, label='Train')
                plt.plot(epochs, df[val_metric], color='red', linewidth=2.5,
        ...
        for row, col, metric, title, color, plot_val in metrics_to_plot:
            ...
            if plot_val:
                ...
                if val_metric in df.columns:
                    ax.plot(epochs, df[val_metric], color='red', linewidth=2, label='Val', linestyle='--', marker='s',
                            markersize=4)
            ...
            if not os.path.exists(save_dir):
                ...
                for base_dir in possible_dirs:
                    if os.path.exists(base_dir):
                        # 查找最新的exp文件夹
                        exp_folders = [f for f in os.listdir(base_dir)
